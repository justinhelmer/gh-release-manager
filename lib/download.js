(function() {
   ...
   * @param {boolean} [options.quiet] - Output nothing.
   * @param {number} [options.recent] - The number of releases to fetch, starting with the most recent.
   * @param {string} [options.repo] - The repository to fetch tags from. If not specified, will be prompted.
   * @param {mixed} [options.verbose] - Output more information. Pass a number to output even more (i.e. verbose=2 shows more than verbose=true).
  ...
  function download(options) {
    ...
    function determineReleases() {
      var getRepo = repo ? _.partial(Promise.resolve, repo) : promptForRepo;
    ...
    function downloadReleases(releases) {
      ...
      function downloadRelease(release) {
        ...
        return response.then(function() {
          if (verbose > 1) {
            console.log('  - ' + chalk.bold.green('[SUCCESS]:'), 'Downloaded to', '\'' + chalk.cyan(dest) + '\'');
          }
        }).catch(function(err) {
          if (verbose > 1) {
