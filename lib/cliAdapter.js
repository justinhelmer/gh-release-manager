(function() {
  ...
  var map = {
    b: {
      ...
      long: 'force',
      helpArgs: ['-f, --force', 'force push to gh-pages during deployment']
    }, h: {
      long: 'head',
      ...
      long: 'no-deploy',
      helpArgs: ['-n, --no-deploy', 'skip the deployment during release (dry run)']
    }, o: {
      long: 'opts',
      ...
      long: 'verbose',
      helpArgs: ['-v, --verbose [n]', 'true for more output; higher number (ie 2) for even more']
    }
  };
  ...
  function prepare(shorts) {
    ...
    _.each(shorts, function(short) {
      var option = map[short].long.replace(/^no-(.+)$/, '$1');
