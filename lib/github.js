(function() {
  ...
  function github(url, stream, options) {
    ...
    if (_.isEmpty(GITHUB_USERNAME) || _.isEmpty(GITHUB_PASSWORD)) {
      getCredentials = promptForCredentials;
    } else {
      getCredentials = _.partial(Promise.resolve, {
        user: GITHUB_USERNAME,
        pass: GITHUB_PASSWORD
      });
    }

    return getCredentials().then(makeRequest);
  ...
  function promptForCredentials() {
    return new Promise(function(resolve) {
      inquirer.prompt([{
        ...
        name: 'pass',
        type: 'password',
        message: 'password (not stored or displayed):',
        validate: _.partial(validate, 'password')
      }], function(answers) {
        resolve({
          user: answers.user,
          pass: answers.pass
